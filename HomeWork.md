...
## 如何理解 option
* 概念
option类似于函数中的参数，通过不通的参数来达成相同的command得到不同的结果.
在cac中，option做为command 的属性存在， 在command执行的时，会读取自身的option属性来做不捅的应对.
* 如何实现的
    option在源码中主要在初始化时对接入的参数进行
## 如何理解 command
command 我个人理解为一个函数一样的存在，能够实现某一种特殊功能的函数. 不同的指令能够实现不同的功能.
## 如何理解 action
action 就是回调函数， 能够在执行后捕捉到执行动作，能够做一些额外的操作
## 如何实现连续调用api
在cac实例对象中调用方法时， 一直返回cac的this，在调用command方法后，返回的是command的实例，在command的方法中返回command实例的this
## Brackets 应该如何使用
在指令名称中使用括号时，尖括号表示必需的指令参数，而方括号表示可选参数。
在选项名称中使用括号时，尖括号表示需要字符串/数字值，而方括号表示该值也可以为 `true`。
## Brackets 是如何实现的
通过正则表达式，对各类括号进行查找和解析， 将解析完毕的结果返回，在`utils.js`中有具体的实现
## Negated Options 是如何实现的
对入参进行Bracket解析后，判断是否以'no-'开头， 有则用negated标识，最后在将对应的参数设置为false
## 分析一下下面这段代码的执行流程
```js
